/**
  ...
  *            - set of firmware functions to manage Leds, push-BT and COM ports
...
/** @defgroup STM324xG_EVAL_LOW_LEVEL 
  * @brief This file provides firmware functions to manage Leds, push-BTs, 
...
GPIO_TypeDef* BT_PORT[BT_NUM] = {WAKEUP_BT_GPIO_PORT, TAMPER_BT_GPIO_PORT,
                                      KEY_BT_GPIO_PORT}; 

const uint16_t BT_PIN[BT_NUM] = {WAKEUP_BT_PIN, TAMPER_BT_PIN,
                                      KEY_BT_PIN}; 

const uint32_t BT_CLK[BT_NUM] = {WAKEUP_BT_GPIO_CLK, TAMPER_BT_GPIO_CLK,
                                      KEY_BT_GPIO_CLK};

const uint16_t BT_EXTI_LINE[BT_NUM] = {WAKEUP_BT_EXTI_LINE,
                                            TAMPER_BT_EXTI_LINE, 
                                            KEY_BT_EXTI_LINE};

const uint16_t BT_PORT_SOURCE[BT_NUM] = {WAKEUP_BT_EXTI_PORT_SOURCE,
                                              TAMPER_BT_EXTI_PORT_SOURCE, 
                                              KEY_BT_EXTI_PORT_SOURCE};
								 
const uint16_t BT_PIN_SOURCE[BT_NUM] = {WAKEUP_BT_EXTI_PIN_SOURCE,
                                             TAMPER_BT_EXTI_PIN_SOURCE, 
                                             KEY_BT_EXTI_PIN_SOURCE}; 
const uint16_t BT_IRQn[BT_NUM] = {WAKEUP_BT_EXTI_IRQn, TAMPER_BT_EXTI_IRQn,
                                       KEY_BT_EXTI_IRQn};
...
/**
  ...
  * @retval None
  */
void STM32F4_EVAL_LEDInit(Led_TpDef Led)
{
...
/**
  ...
  * @retval None
  */
void STM32F4_EVAL_LEDOn(Led_TpDef Led)
{
...
/**
  ...
  * @retval None
  */
void STM32F4_EVAL_LEDOff(Led_TpDef Led)
{
...
/**
  ...
  * @retval None
  */
void STM32F4_EVAL_LEDToggle(Led_TpDef Led)
{
//   GPIO_PORT[Led]->ODR ^= GPIO_PIN[Led];
}

/**
  * @brief  Configures BT GPIO and EXTI Line.
  * @param  BT: Specifies the BT to be configured.
  *   This parameter can be one of following parameters:   
  *     @arg BT_WAKEUP: Wakeup Push BT
  *     @arg BT_TAMPER: Tamper Push BT  
  *     @arg BT_KEY: Key Push BT 
  *     @arg BT_RIGHT: Joystick Right Push BT 
  *     @arg BT_LEFT: Joystick Left Push BT 
  *     @arg BT_UP: Joystick Up Push BT 
  *     @arg BT_DOWN: Joystick Down Push BT
  *     @arg BT_SEL: Joystick Sel Push BT
  * @param  BT_Mode: Specifies BT mode.
  *   This parameter can be one of following parameters:   
  *     @arg BT_MODE_GPIO: BT will be used as simple IO 
  *     @arg BT_MODE_EXTI: BT will be connected to EXTI line with interrupt
  *                     generation capability  
  * @retval None
  */
void STM32F4_EVAL_PBInit(BT_TpDef BT, BTMode_TpDef BT_Mode)
{
  GPIO_InitTypeDef GPIO_InitStructure;
  ...
  /* Enable the BT Clock */
  RCC_AHB1PeriphClockCmd(BT_CLK[BT], ENABLE);
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG, ENABLE);

  /* Configure BT pin as input */
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
  GPIO_InitStructure.GPIO_Pin = BT_PIN[BT];
  GPIO_Init(BT_PORT[BT], &GPIO_InitStructure);


  if (BT_Mode == BT_MODE_EXTI)
  {
    /* Connect BT EXTI Line to BT GPIO Pin */
    SYSCFG_EXTILineConfig(BT_PORT_SOURCE[BT], BT_PIN_SOURCE[BT]);

    /* Configure BT EXTI line */
    EXTI_InitStructure.EXTI_Line = BT_EXTI_LINE[BT];
    ...
    /* Enable and set BT EXTI Interrupt to the lowest priority */
    NVIC_InitStructure.NVIC_IRQChannel = BT_IRQn[BT];
...
/**
  * @brief  Returns the selected BT state.
  * @param  BT: Specifies the BT to be checked.
  *   This parameter can be one of following parameters:    
  *     @arg BT_WAKEUP: Wakeup Push BT
  *     @arg BT_TAMPER: Tamper Push BT  
  *     @arg BT_KEY: Key Push BT 
  ...
  *     @arg BT_DOWN: Joystick Down Push BT
  *     @arg BT_SEL: Joystick Sel Push BT    
  * @retval The BT GPIO pin value.
  */
uint32_t STM32F4_EVAL_PBGetState(BT_TpDef BT)
{
  return GPIO_ReadInputDataBit(BT_PORT[BT], BT_PIN[BT]);
}
...
/**
  ...
  *     @arg COM1
  *     @arg COM2  
  * @param  Struct: pointer to a TypeDef structure that
